...
class Trainer(object):
    ...
    def make_kwargs(self, indices, input_seq, target_seq, gv_feat, att_feats, att_mask):
        ...
        kwargs = {
            ...
            'is_training': self.model.training  # 直接使用字符串常量
        }
    ...
    def forward(self, kwargs):
        if self.rl_stage == False:
            ...
            if isinstance(model_output, tuple) and len(model_output) == 3:
                logit, continuity_loss, similarity_loss = model_output
                aux_loss = continuity_loss + similarity_loss
            elif isinstance(model_output, tuple) and len(model_output) == 2:
                logit, continuity_loss = model_output
                similarity_loss = 0
                aux_loss = continuity_loss
            else:
                logit = model_output
                continuity_loss = 0
                similarity_loss = 0
            ...
            loss_info['continuity_loss'] = continuity_loss.item() if isinstance(continuity_loss,
                                                                                torch.Tensor) else continuity_loss
            loss_info['similarity_loss'] = similarity_loss.item() if isinstance(similarity_loss,
                                                                                torch.Tensor) else similarity_loss
            loss_info['aux_loss'] = aux_loss.item() if isinstance(aux_loss, torch.Tensor) else aux_loss
    ...
    def train(self):
        ...
        for epoch in range(self.load_epoch + 1, cfg.SOLVER.MAX_EPOCH):
            ...
            running_loss = .0
            running_continuity_loss = .0  # 新增：监控连续性损失
            running_similarity_loss = .0  # 新增：监控相似性损失
            running_aux_loss = .0  # 新增：监控辅助损失
            ...
            with tqdm.tqdm(desc='Epoch %d - train' % epoch, unit='it', total=len(self.training_loader)) as pbar:
                for _, (indices, input_seq, target_seq, gv_feat, att_feats, att_mask) in enumerate(self.training_loader):
                    ...
                    # 记录各种损失
                    if 'continuity_loss' in loss_info:
                        running_continuity_loss += loss_info['continuity_loss']
                    if 'similarity_loss' in loss_info:
                        running_similarity_loss += loss_info['similarity_loss']
                    if 'aux_loss' in loss_info:
                        running_aux_loss += loss_info['aux_loss']

                    if not self.rl_stage:
                        pbar.set_postfix({
                            'loss': '%.3f' % (running_loss / (_ + 1)),
                            'cont': '%.4f' % (running_continuity_loss / (_ + 1)),
                            'sim': '%.4f' % (running_similarity_loss / (_ + 1)),
                            'aux': '%.4f' % (running_aux_loss / (_ + 1))
                        })
                    else:
                        # SCST训练时的显示
