...
from models.M1.RegionAggregationModule import RegionAggregationModule as RAM
from models.M1.TextEnhancedRegionModule import TextEnhancedRegionModule as TERM
from models.M2.RegionToGridFusionModule import RegionToGridFusionModule as RTGFM

class CLIPGlobalFeatureExtractor(nn.Module):
    def __init__(self, clip_model_name="ViT-B/32"):
        super(CLIPGlobalFeatureExtractor, self).__init__()
        # self.clip_model, _ = clip.load(clip_model_name)
        self.clip_model, _ = clip.load(clip_model_name, download_root="/24085403002/24085403002/SMART/clipmodel/")      # CLIP模型文件路径
    ...
    def forward(self, x):
        # 调整输入尺寸到CLIP期望的224x224
        x = F.interpolate(x, size=(224, 224), mode='bicubic', align_corners=False)

        # 使用CLIP提取全局特征
        with torch.no_grad():
            global_feature = self.clip_model.encode_image(x)
...
class SMART(BasicModel):
    def __init__(self):
        ...
        #===============================================================================================================
        # 使用CLIP提取全局特征
        self.clip_extractor = CLIPGlobalFeatureExtractor("ViT-B/32")
        # ==============================================================================================================
        ...
        self.RegionAggregation = RAM(
            num_regions = 64,           #聚合区域数量 ***
            dim = 512,
            alpha = 50.0,
            normalize_input=True,
            continuity_weight = 0.5,      # 一致性损失权重 ***
            continuity_kernel = 3       # 3x3连续性检测核
        )

        # Text Enhance
        self.TextEnhancer = TERM(
            region_dim = 512,
            text_dim = 512,
            num_heads = 8,
            text_embeddings_path="/24085403002/24085403002/SMART/stuff_ViT16_clip_text.pth",  # 预训练文本特征路径
            text_categories=171,        # 文本类别数
            use_alignment=True,
            alpha_init=0.7              # 对齐模块中的初始融合权重 ***
        )

        # RegionToGrid Fusion
        self.R2GFusion = RTGFM(
            dim = 512,
            num_regions = 64,          # 与RegionAggregationModule一致
            base_topk=16,               # ***
            min_topk=8,                 # ***
            max_topk=32,                # ***
            use_attention=True,         # 使用协同注意力 ***
            attn_heads=8,
            attn_fusion = "weighted"    # ***
        )
        # ==============================================================================================================
    ...
    def forward(self, **kwargs):
        att_feats = kwargs[cfg.PARAM.ATT_FEATS] # 10, 3, 384, 384
        seq = kwargs[cfg.PARAM.INPUT_SENT]      # 50, 17

        # 使用CLIP提取全局特征
        # 确保CLIP特征使用半精度（如果模型在其他地方使用半精度）
        clip_global_features = self.clip_extractor(att_feats).type(torch.float32)  # [10, 512] datatype:float16->32

        # backbone forward
        att_feats = self.backbone(att_feats)    # 10, 144, 1536
        att_feats = self.att_embed(att_feats)  # [10, 144, 512]

        # region aggregation
        B = att_feats.size(0)
        pseudo_regions, continuity_loss = self.RegionAggregation(att_feats.view(B, 12, 12, -1))

        # text enhance
        enhanced_pseudo_regions = self.TextEnhancer(pseudo_regions, clip_global_features)

        # 计算相似度损失
        if self.TextEnhancer.text_embeddings is not None:
            text_embeddings = self.TextEnhancer.text_embeddings.to(enhanced_pseudo_regions.device)  # 确保文本嵌入在正确的设备上
            # 计算相似度损失
            similarity_loss = self.TextEnhancer.calculate_similarity_loss(enhanced_pseudo_regions, text_embeddings)
            similarity_weight = 0.1         # ***
            similarity_loss = similarity_weight * similarity_loss
        else:
            similarity_loss = 0

        # fusion region grid
        grid_features = att_feats
        fused_features = self.R2GFusion(grid_features, enhanced_pseudo_regions)

        # expand
        fused_features = utils.expand_tensor(fused_features,  cfg.DATA_LOADER.SEQ_PER_IMG)

        # grid_cls -> gx
        grid_cls = utils.expand_tensor(clip_global_features.unsqueeze(dim=1), cfg.DATA_LOADER.SEQ_PER_IMG)  # 50, 1, 512
        ...
        gx, encoder_out = self.encoder(fused_features, grid_cls, att_mask)
        decoder_out = self.decoder(gx, seq, encoder_out, seq_mask, att_mask)

        # 只在训练时返回辅助损失
        if kwargs.get('is_training', False):
            return F.log_softmax(decoder_out, dim=-1), continuity_loss, similarity_loss
        else:
    ...
    def decode_beam(self, **kwargs):
        ...
        #===============================================================================================================
        clip_global_features = self.clip_extractor(att_feats).type(torch.float32)
        att_feats = self.backbone(att_feats)
        att_feats = self.att_embed(att_feats)
        B = att_feats.size(0)
        pseudo_regions, _ = self.RegionAggregation(att_feats.view(B, 12, 12, -1))
        enhanced_pseudo_regions = self.TextEnhancer(pseudo_regions, clip_global_features)
        grid_features = att_feats
        fused_features = self.R2GFusion(grid_features, enhanced_pseudo_regions)
        grid_cls = clip_global_features.unsqueeze(dim=1)
        # ==============================================================================================================
